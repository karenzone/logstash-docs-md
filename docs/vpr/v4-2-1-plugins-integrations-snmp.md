...
mapped_pages:
  - https://www.elastic.co/guide/en/logstash-versioned-plugins/current/v4.2.1-plugins-integrations-snmp.html
applies_to:
  stack: ga
---

# SNMP Integration Plugin v4.2.1 [v4.2.1-plugins-integrations-snmp]
...
* [Migrating to `logstash-integration-snmp` from individual plugins](v4-2-1-plugins-integrations-snmp.md#v4.2.1-plugins-integrations-snmp-migration)
* [Importing MIBs](v4-2-1-plugins-integrations-snmp.md#v4.2.1-plugins-integrations-snmp-import-mibs)

## Migrating to `logstash-integration-snmp` from individual plugins [v4.2.1-plugins-integrations-snmp-migration]
...
### Migration notes: `logstash-input-snmp` [v4.2.1-plugins-integrations-snmp-migration-input-snmp]
...
### Changes to mapping and error logging: `logstash-input-snmp` [v4.2.1-plugins-integrations-snmp-input-snmp-mapping]
...
### Migration notes: `logstash-input-snmptrap` [v4.2.1-plugins-integrations-snmp-migration-input-snmptrap]
...
### Changes to mapping and error logging: `logstash-input-snmptrap` [v4.2.1-plugins-integrations-snmp-input-snmptrap-mapping]
...
### Maintain maximum compatibility with previous implementation [v4.2.1-plugins-integrations-snmp-input-snmptrap-compat]
...
## Importing MIBs [v4.2.1-plugins-integrations-snmp-import-mibs]
...
When `mib_paths` is configured, note that the complete set of all MIBs required must be included here. Any conflicting MIBs with defaults will prefer the default, not the MIB configured on the `mib_paths`. The tightest control and maximal compatability can be acheived by ensuring that `use_provided_mibs` is set to `false` and the complete set of MIBs are included in files listed in `mib_paths`. The defaults from the old implementation can be found in the [ruby-snmp github repository](https://github.com/ruby-snmp/ruby-snmp/tree/master/data/ruby/snmp/mibs).

### Preventing a `failed to locate MIB module` error [v4.2.1-plugins-integrations-snmp-locate-mibs]
...
### Option 1: Use an environment variable [v4.2.1-plugins-integrations-snmp-env-var]
...
### Option 2: Provide a configuration file [v4.2.1-plugins-integrations-snmp-mib-config]
